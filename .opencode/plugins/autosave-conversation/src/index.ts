...
async function handleEvent(
  ...
  switch (event.type) {
    ...
    case 'session.deleted':
      await handleSessionDeleted(event as SessionDeletedEvent, client, directory, saveDir);
      break;
  }
}
  ...
  try {
    ...
    if (!title || title.startsWith('New-session-') || title.startsWith('New session')) {
      ...
      if (firstUserMessage) {
        ...
        if (firstTextPart) {
          title = extractTopicFromMessage(firstTextPart.text, DEFAULT_CONFIG.maxTopicLength);
          session.title = title;
        }
      }
    }
    ...
    const content = formatSession(title, session.createdAt, messages);
    ...
    for (const part of message.parts) {
      if (part.type === 'file' && 'url' in part && 'mime' in part) {
        ...
        if (filePart.mime.startsWith('image/') && isBase64ImageUrl(filePart.url)) {
          const localPath = await saveImageFromBase64(
            filePart.url,
            mdFilePath,
            sessionTitle,
            createdAt,
